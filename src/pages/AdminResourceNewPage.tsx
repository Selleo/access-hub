...
export function AdminResourceNewPage() {
  ...
  const [roles, setRoles] = useState<RoleDraft[]>([
    { name: "User", description: "", requiresApproval: false },
    { name: "Owner", description: "", requiresApproval: true },
  ]);
  const [submitting, setSubmitting] = useState(false);
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const addRole = () => {
    setRoles((prev) => [...prev, { name: "", description: "", requiresApproval: false }]);
  };
        ...
        <div className="mt-5 border-t border-[#f0f1f5] pt-4">
          ...
          <div className="space-y-2">
            {roles.map((role, index) => (
              <div key={`${role.name}-${index}`} className="rounded-xl border border-[#e7eaf2] p-3">
